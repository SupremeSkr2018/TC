    ...
    <div class="content">
      ...
      <div v-if="step === 2">
        ...
        <div class="block">
          <p>请选择成型工:</p>
          <div class="level">
            <div v-for="(item, index) in selectedWorkerList" :key="item.code">
              <div
                class="btn bgcGreen"
                ...
                :style="{'line-height': 'unset', height: '80px', 'padding': '8px 15px 0'}"
              >
                <div :style="{'font-size': '30px'}">{{ item.code }}</div>
                <div :style="{'font-size': '14px'}">{{ item.name }}</div>
              </div>
...
export default {
  ...
  methods: {
    ...
    checkPostLocalData() {
      let that = this;
      localforage
        .getItem("local7" + that.baseURL + that.userCode)
        .then(function(value) {
          ...
          res => {
            ...
            if (res && res.status == 200) {
              ...
              if (num == len) {
                ...
                localforage
                  .removeItem("local7" + that.baseURL + that.userCode)
                  .then(function() {})
                  .catch(function() {});
                that.localData = null;
              }
        ...
        res => {
          if (res && res.status == 200) {
            ...
            for (const item of result) {
              ...
              if (responsible[item.description]){
                responsible[item.job_pos_name].push({ name:item.name })
              }
            }
    ...
    toastConfirm(n) {
      ...
      if(this.ware == '青坯库半检车间') that.sloc = 2;
      if(this.ware == '青坯库施釉车间') that.sloc = 3;
      if(this.ware == '') that.sloc = null;
          ...
          localforage
            ...
            .then(function(value) {
              ...
              if (!res) {
                ...
                localforage
                  .setItem(
                    "local7" + that.baseURL + that.userCode,
                    JSON.stringify(arr)
                  )
                  .then(function() {
                    // 当值被存储后，可执行其他操作
    ...
    selectProCode: function(value) {
      ...
      let i = this.proCodeShowList.filter(a => a != value);
      i.map(res => {
        if (res.number > 0) {
          this.workerList[res].map(item => {
      ...
      this.selectedWorkerList = this.workerList[this.selectedProCode] || [];
      this.operateWorkerIndex = "";      
    },
    selectWorker: function(value,index) {
      this.selectedWorker = value
      this.operateWorkerIndex = index;
