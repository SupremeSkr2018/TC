  ...
  <div class="wrap">
    ...
    <div class="content">
      ...
      <div>
        <div class="block">
          ...
          <ul class="clearfix step2">
            <li
              ...
              :key="item.id"
              class="btn"
              :class="{bgcRed: item.color_name === selectedColor}"
        ...
        <div class="block">
          <p>请选择商标:</p>
          <ul class="clearfix step2" v-show="tradeShow">
            <li
              v-for="(item) in tradeMarkList"
              :key="item.clientFullName"
              class="btn"
              :class="{bgcRed: item === selectedTradeMark}"
              @click="selectTradeMark(item)"
            >{{ item.clientFullName }}</li>
          </ul>
        </div>
        <div class="block">
  ...
  methods: {
    ...
    toastConfirm(n) {
      ...
      let param = {
        ...
        // workCode: null, //工号
        // workName: null, //工人姓名
        trademark: that.selectedTradeMark.clientFullName,    //商标
          ...
          localforage
            ...
            .then(function(value) {
              ...
              if (!res) {
                ...
                localforage
                  .setItem(
                    "local13" + that.baseURL + that.userCode,
                    JSON.stringify(arr)
                  )
                  .then(function() {
                    // 当值被存储后，可执行其他操作
    ...
    selectTradeMark: function(value) {
      ...
      let data = {
        ...
        colorCode:that.color,
        formCode:'020',
        trademark:that.selectedTradeMark.clientFullName,
        slocId:4        
      }  
